...
对秩的定义有很多，例如：树高 / 集合的大小，按秩合并是一种启发式合并，将 “小的结构” 合并到 “大的结构” 上，这是使用树高作为秩的定义，也即将较矮的树合并到较高的树，直接将较低的树的根结点的父结点赋值为较高的树的根结点，这样合并后，较低的树每次合并后树高 +$1$ 。这样在平均情况下，整体的时间复杂度会更小，对于一般的并查集，只使用按秩合并的话，将所有集合合并到一起的时间复杂度是 $O(nlog \ n)$ 那么每次**查询操作**的时间复杂度为 $O(log \ n)$.
