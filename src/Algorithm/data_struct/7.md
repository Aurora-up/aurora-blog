...
并查集本身是由若干树组成的森林，可以在树中**每条边上记录一个权值**，在代码实现中，维护一个数组 $d$ ，用 $d[x]$ 保存结点 $x$ 到父结点 $fa[x]$ 之间的边权。在每次路径压缩过后，每个访问的结点都会直接指向树根，如果在路径压缩过程中同时更新这些结点的 $d$ 值，也即利用路径压缩的过程来统计每个结点到树根之间的路径上的一些信息。
